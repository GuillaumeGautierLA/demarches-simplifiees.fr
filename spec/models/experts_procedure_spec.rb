  ...
  describe '#invited_experts' do
    ...
    context 'when there are two dossiers' do
      ...
      context 'and each one has an avis from 3 different experts' do
        ...
        it { is_expected.to eq([experts_procedure, experts_procedure2, experts_procedure3]) }
        it { expect(procedure.experts.count).to eq(3) }
        it { expect(procedure.experts).to eq([expert, expert2, expert3]) }
      end
    end
  end
end
