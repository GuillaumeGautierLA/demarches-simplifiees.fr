describe FranceConnectInformation, type: :model do
  ...
  describe 'associate_user!' do
    ...
    context 'when instructeur with same email already exist' do
      let(:instructeur) { create(:instructeur) }
      let(:email) { instructeur.email }
      let(:subject) { build(:france_connect_information, email_france_connect: email) }

      it { expect(subject.associate_user!).to be_falsey }
    end

    context 'when admin with same email already exist' do
